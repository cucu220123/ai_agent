...
以下结果来自 `scripts/run_demo.py` 在 1200 行模拟客户流失数据上的一次运行。
...
最终选择 Logistic Regression：满足 ROC-AUC ≥ 0.75，运行耗时较低，并且对运营侧更易解释。验证器同时通过了静态安全、接口导入、训练/预测功能、输出契约、指标门槛、稳定性和运行时间检查。
